...
export function validateGeneratedStory(repo, generated) {
  ...
  (generated.actions || []).forEach((rawAction, actionIndex) => {
    ...
    if (!steps.some((step) => implementationDetails[actionId][step.nodeId].evidenceType === "SOURCE")) {
      for (const step of steps) delete implementationDetails[actionId][step.nodeId];
      delete implementationDetails[actionId];
      for (const step of steps) nodes.pop();
      while (edges.length && edges[edges.length - 1].id.startsWith(`${actionId}-edge-`)) edges.pop();
      return;
    }
