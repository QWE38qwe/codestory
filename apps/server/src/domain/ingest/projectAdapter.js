export function toRuntimeProject({ repo, featureDraft, validated }) {
  ...
  return {
    ...
    repoMeta: { origin: repo.origin, commit: repo.commit, stats: repo.stats, generatedAt: new Date().toISOString(), evidenceSummary: { sourceSteps: Object.values(validated.implementationDetails).flatMap((items) => Object.values(items)).filter((item) => item.evidenceType === "SOURCE").length } },
  };
}
